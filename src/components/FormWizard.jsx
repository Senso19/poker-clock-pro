...
/**
 * FormWizard — moteur générique de rendu d'un formulaire multi-pages
 * (contenu + champs + boutons de navigation/embranchement). Réutilisé par
 * PublicRegistrationForm (soumission réelle) et FormTemplatePreviewModal
...
export default function FormWizard({ pages, theme: rawTheme, isOpen = true, onSubmit, banner = null }) {
  ...
  if (!isOpen) {
    return (
      <FormShell theme={theme} banner={banner}>
  ...
  if (!pages || pages.length === 0) {
    return (
      <FormShell theme={theme} banner={banner}>
        Ce formulaire n'a pas encore de contenu.
      </FormShell>
    );
  }
...
function FormShell({ theme, children, banner }) {
  const bg = theme.bgColor || "#14181C";
  const card = theme.cardColor || "#1B2027";
  const text = theme.textColor || "#EDEAE3";
  return (
    <div style={{ backgroundColor: bg, minHeight: "100%" }} className="flex items-center justify-center p-4 font-body relative">
      {banner}
